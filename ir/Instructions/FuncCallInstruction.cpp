...
namespace {
	Type * get_array_argument_type(ArrayType * arrayType)
	{
		if (!arrayType) {
			return nullptr;
		}
...
void FuncCallInstruction::toString(std::string & str)
{
	...
	if (argCount == 0) {
		...
		for (int32_t k = 0; k < operandsNum; ++k) {
			...
			auto * arrayType = dynamic_cast<ArrayType *>(operandType);
			if (arrayType) {
				Type * argType = get_array_argument_type(arrayType);
				str += argType->toString() + " " + operand->getIRName();
			} else {
				str += operandType->toString() + " " + operand->getIRName();
			}
