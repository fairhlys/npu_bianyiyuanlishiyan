...
public:
	...
	{
		if (this->addrValue) {
			int32_t addrReg = this->addrValue->getLoadRegId();
			if (addrReg == -1) {
				addrReg = this->addrValue->getRegId();
			}
	...
	/// @param _offset 偏移
	///
	void setMemoryAddr(Value * addr, int64_t _offset)
	{
		this->addrValue = addr;
		offset = _offset;
		baseRegNo = -1;
	...
	///
	/// @brief 获取动态地址表达式
	/// @return Value* 动态地址值；若为空则表示静态基址寻址
	///
	[[nodiscard]] Value * getAddrValue() const
	{
	...
	/// @return int64_t 偏移值
	///
	[[nodiscard]] int64_t getAddrOffset() const
	{
		return this->offset;
	}
