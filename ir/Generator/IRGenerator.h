...
protected:
	...
	/// @brief 布尔表达式节点翻译成线性中间IR（结果归一化为0/1）
	/// @param node AST节点
	/// @return 翻译是否成功，true：成功，false：失败
	bool ir_bool_expr(ast_node * node);

	/// @brief if/if-else语句翻译
	...
	/// @brief 生成条件分支代码（短路求值）
	bool
	emitCondBranch(ast_node * condNode, LabelInstruction * trueLabel, LabelInstruction * falseLabel, InterCode & out);
...
private:
	...
	/// @brief 循环的break目标栈
	std::vector<LabelInstruction *> breakTargetStack;

	/// @brief 循环的continue目标栈
	std::vector<LabelInstruction *> continueTargetStack;
};
