...
namespace {
	std::string format_decl_type_and_name(Type * type, const std::string & irName)
	{
		if (type && type->isArrayType()) {
			auto * arrayType = dynamic_cast<ArrayType *>(type);
			if (arrayType) {
				std::string s = arrayType->getElementType()->toString() + " " + irName;
				for (auto dim: arrayType->getDims()) {
					s += "[" + std::to_string(dim) + "]";
				}
				return s;
			}
	...
	std::string format_param_type_and_name(Type * type, const std::string & irName)
	{
		if (type && type->isArrayType()) {
			auto * arrayType = dynamic_cast<ArrayType *>(type);
			if (arrayType) {
...
void Function::toString(std::string & str)
{
	...
	for (auto & param: params) {
		...
		std::string param_str = format_param_type_and_name(param->getType(), param->getIRName());
	...
	for (auto & var: this->varsVector) {
		...
		str += "\tdeclare " + format_decl_type_and_name(var->getType(), var->getIRName());
	...
	// 输出内存变量（如数组元素左值）的声明
	for (auto & mem: this->memVector) {
		str += "\tdeclare " + format_decl_type_and_name(mem->getType(), mem->getIRName()) + "\n";
	}
	...
	for (auto & inst: code.getInsts()) {
		...
		if (inst->hasResultValue()) {
			...
			// 局部变量和临时变量需要输出declare语句
			str += "\tdeclare " + format_decl_type_and_name(inst->getType(), inst->getIRName()) + "\n";
		}
	}
...
void Function::renameIR()
{
	...
	// 内存变量重命名（数组元素访问等）
	for (auto & mem: this->memVector) {
		mem->setIRName(IR_LOCAL_VARNAME_PREFIX + std::to_string(nameIndex++));
	}
