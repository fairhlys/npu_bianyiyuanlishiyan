...
{
	std::cout << exeName + " -S [--symbol] [-D | --recursive-descent] [-T | --ast | -I | --ir] [-o "
	...
	std::cout << "  -C, --cfg                  Output control flow graph\n";
...
static int ArgsAnalysis(int argc, char * argv[])
{
	...
	// -D指定按照递归下降分析法执行，不指定时按flex+bison执行
	// -o要求必须带有附加参数，指定输出的文件
	// -O要求必须带有附加整数，指明优化的级别
	// -t要求必须带有目标CPU，指明目标CPU的汇编
	// -c选项在输出汇编时有效，附带输出IR指令内容
	const char options[] = "ho:STICDO:t:c";
	int option_index = 0;
...
lb_check:
	...
	int flag = (int) gShowLineIR + (int) gShowAST + (int) gShowCFG;
