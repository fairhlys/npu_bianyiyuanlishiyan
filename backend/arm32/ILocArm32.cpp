...
{
	int32_t loadRegId = src_var->getLoadRegId();
	if (loadRegId != -1) {
		if (loadRegId != rs_reg_no) {
			emit("mov", PlatformArm32::regName[rs_reg_no], PlatformArm32::regName[loadRegId]);
		}
		return;
	}
	...
	if (Instanceof(constVal, ConstInt *, src_var)) {
		...
		if (auto * memVar = dynamic_cast<MemVariable *>(src_var); memVar && memVar->getAddrValue()) {
			load_var(rs_reg_no, memVar->getAddrValue());
			load_base(rs_reg_no, rs_reg_no, memVar->getAddrOffset());
			return;
		}
...
void ILocArm32::lea_var(int rs_reg_no, Value * var)
{
	...
	if (auto * memVar = dynamic_cast<MemVariable *>(var); memVar && memVar->getAddrValue()) {
		load_var(rs_reg_no, memVar->getAddrValue());
		if (memVar->getAddrOffset() != 0) {
			leaStack(rs_reg_no, rs_reg_no, memVar->getAddrOffset());
		}
		return;
	}
...
void ILocArm32::store_var(int src_reg_no, Value * dest_var, int tmp_reg_no)
{
	...
	if (dest_var->getRegId() != -1) {
		...
		if (auto * memVar = dynamic_cast<MemVariable *>(dest_var); memVar && memVar->getAddrValue()) {
			load_var(tmp_reg_no, memVar->getAddrValue());
			store_base(src_reg_no, tmp_reg_no, memVar->getAddrOffset(), tmp_reg_no);
			return;
		}
