...
InstSelectorArm32::InstSelectorArm32(
	vector<Instruction *> & _irCode, ILocArm32 & _iloc, Function * _func, SimpleRegisterAllocator & allocator)
	: ir(_irCode), iloc(_iloc), func(_func), simpleRegisterAllocator(allocator)
{
	...
	translator_handlers[IRInstOperator::IRINST_OP_BR] = &InstSelectorArm32::translate_br;
...
{
	Instanceof(brInst, CondBrInstruction *, inst);

	std::string branchOp = "bne";
	if (brInst->getCond() == IRBrCond::EQ) {
		branchOp = "beq";
	} else if (brInst->getCond() == IRBrCond::NE) {
		branchOp = "bne";
	} else if (brInst->getCond() == IRBrCond::LT) {
		branchOp = "blt";
	} else if (brInst->getCond() == IRBrCond::LE) {
		branchOp = "ble";
	} else if (brInst->getCond() == IRBrCond::GT) {
		branchOp = "bgt";
	} else if (brInst->getCond() == IRBrCond::GE) {
		branchOp = "bge";
	}

	Value * lhs = inst->getOperand(0);
	int32_t lhsReg = lhs->getRegId();
	int32_t loadLhsReg;
	if (lhsReg == -1) {
		loadLhsReg = simpleRegisterAllocator.Allocate(lhs);
		iloc.load_var(loadLhsReg, lhs);
	} else {
		loadLhsReg = lhsReg;
	}

	if (brInst->isCompareMode()) {
		Value * rhs = inst->getOperand(1);
		int32_t rhsReg = rhs->getRegId();
		int32_t loadRhsReg;
		if (rhsReg == -1) {
			loadRhsReg = simpleRegisterAllocator.Allocate(rhs);
			iloc.load_var(loadRhsReg, rhs);
		} else {
			loadRhsReg = rhsReg;
		}

		iloc.inst("cmp", PlatformArm32::regName[loadLhsReg], PlatformArm32::regName[loadRhsReg]);
		simpleRegisterAllocator.free(rhs);
	} else {
		// 默认按非零分支
		iloc.inst("cmp", PlatformArm32::regName[loadLhsReg], "#0");
	}

	iloc.inst(branchOp, brInst->getTrueTarget()->getName());
	iloc.jump(brInst->getFalseTarget()->getName());
...
void InstSelectorArm32::translate_assign(Instruction * inst)
{
	...
	auto * resultMem = dynamic_cast<MemVariable *>(result);

	if (resultMem && result->getType() && result->getType()->isPointerType() && arg1->getType() &&
		arg1->getType()->isPointerType()) {
		// 这类赋值只用于把动态地址表达式绑定到 MemVariable，后端不需要再生成真实写内存指令。
		return;
	}

	if (arg1->getType() && arg1->getType()->isArrayType()) {
		auto * arrayType = dynamic_cast<ArrayType *>(arg1->getType());
		if (arrayType && arrayType->isParameterArray()) {
			// 参数数组本质上传递的是首地址，这里需要搬运指针值本身，而不是再取一次地址。
		} else {
...
void InstSelectorArm32::translate_mod_int32(Instruction * inst)
{
	...
	int32_t quotient_reg_no = simpleRegisterAllocator.Allocate();
	int32_t product_reg_no = simpleRegisterAllocator.Allocate();
	...
	iloc.inst(
		"mul",
		PlatformArm32::regName[product_reg_no],
		PlatformArm32::regName[quotient_reg_no],
		PlatformArm32::regName[load_arg2_reg_no]);
	...
	simpleRegisterAllocator.free(product_reg_no);
	simpleRegisterAllocator.free(arg1);
	simpleRegisterAllocator.free(arg2);
	simpleRegisterAllocator.free(result);
}
