...
Module::Module(std::string _name) : name(_name)
{
	...
	(void) newFunction(
		...
		{new FormalParam{const_cast<PointerType *>(PointerType::get(IntegerType::getTypeInt())), ""}},
		true);
	(void) newFunction(
		"putarray",
		VoidType::getType(),
...
/// @brief 将已有Value插入到当前作用域
/// @param value Value对象
void Module::insertValue(Value * value)
{
	scopeStack->insertValue(value);
}

void Module::insertValueAs(const std::string & name, Value * value)
{
	scopeStack->insertValueAs(name, value);
}
...
GlobalVariable * Module::newUniqueGlobalVariable(Type * type, std::string baseName)
{
	if (baseName.empty()) {
		baseName = "__sg";
	}
	...
	do {
		uniqueName = baseName + "_" + std::to_string(staticLocalNameIndex++);
	} while (findGlobalVariable(uniqueName) != nullptr);

	return newGlobalVariable(type, uniqueName);
}

Value * Module::newStaticLocalVarValue(Type * type, std::string localName)
{
	if (!currentFunc || localName.empty()) {
		return nullptr;
	}

	if (scopeStack->findCurrentScope(localName) != nullptr) {
		minic_log(LOG_ERROR, "变量(%s)已经存在", localName.c_str());
		return nullptr;
	}

	std::string baseName = "__s_" + currentFunc->getName() + "_" + localName;
	GlobalVariable * gv = newUniqueGlobalVariable(type, baseName);
	insertValueAs(localName, gv);
	return gv;
}

void Module::registerGlobalInit(GlobalVariable * var, int32_t initVal)
{
	if (!var) {
		return;
	}
	globalInitList.emplace_back(var, initVal);
}

std::vector<std::pair<GlobalVariable *, int32_t>> Module::consumeGlobalInits()
{
	auto list = globalInitList;
...
/// @brief 基于线性IR构建并优化CFG，回写到各函数的线性IR中
void Module::optimizeIR()
{
	for (auto func: funcVector) {
...
/// @brief 输出控制流图CFG
void Module::outputCFG(const std::string & filePath)
{
#ifdef USE_GRAPHVIZ
	GVC_t * gv = gvContext();
	...
	for (auto func: funcVector) {
		...
		std::string clusterName = "cluster_" + func->getIRName();
		Agraph_t * subGraph = agsubg(g, const_cast<char *>(clusterName.c_str()), 1);
		if (!subGraph) {
			continue;
		}

		agsafeset(subGraph, (char *) "label", const_cast<char *>(func->getIRName().c_str()), (char *) "");
		agsafeset(subGraph, (char *) "style", (char *) "rounded", (char *) "");

		ControlFlowGraph cfg(func);
		cfg.addToGraph(subGraph, func->getIRName());
	}
