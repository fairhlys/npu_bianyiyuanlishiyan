...
public:
	...
	/// @brief 新建静态局部变量（底层按全局变量存储），并按局部名插入当前作用域
	Value * newStaticLocalVarValue(Type * type, std::string localName);

	/// @brief 新建唯一命名的全局变量
	GlobalVariable * newUniqueGlobalVariable(Type * type, std::string baseName);

	/// @brief 登记全局变量的常量初始化
	void registerGlobalInit(GlobalVariable * var, int32_t initVal);

	/// @brief 取出并清空全局初始化清单
	std::vector<std::pair<GlobalVariable *, int32_t>> consumeGlobalInits();
	...
	/// @brief 以指定名字插入到当前作用域
	void insertValueAs(const std::string & name, Value * value);
	...
	/// @brief 输出控制流图CFG
	/// @param filePath
	void outputCFG(const std::string & filePath);
...
private:
	...
	/// @brief 静态局部变量唯一命名计数器
	int32_t staticLocalNameIndex = 0;

	/// @brief 全局变量初始化清单（在main入口注入）
	std::vector<std::pair<GlobalVariable *, int32_t>> globalInitList;
};