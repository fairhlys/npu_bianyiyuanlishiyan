...
static ast_node * parseArrayDims();
...
///
/// @brief 表达式文法 expr : addExp, 表达式目前支持加减乘除模运算
/// @return AST的节点
///
static ast_node * expr()
{
...
static ast_node * statement()
{
	...
	if (F(T_RETURN)) {
		...
		advance();
	} else if (F(T_ID) _(T_L_PAREN) _(T_DIGIT) _(T_SUB)) {
...
///
/// @brief 函数定义的识别，其文法为： funcDef -> T_INT T_ID idtail
...
/// 因此改造后的文法为：
/// 文法：compileUnit -> { funcDef } EOF
/// funcDef: T_INT T_ID idtail
/// idtail : varDeclList | T_L_PAREN T_R_PAREN block
/// varDeclList : T_COMMA T_ID varDeclList | T_SEMICOLON
...
static ast_node * idtail(type_attr & type, var_id_attr & id)
{
	...
	// 解析数组维度（如果有）
	ast_node * dimsNode = parseArrayDims();

	// 根据第一个变量声明创建变量声明语句节点并加入其中
	ast_node * stmt_node;
	if (dimsNode) {
		stmt_node = ast_node::create_var_decl_stmt_node(type, id, dimsNode);
	} else {
		stmt_node = ast_node::create_var_decl_stmt_node(type, id);
	}
...
///
/// @brief 识别ID尾部符号，可以是括号，代表函数调用；可以是中括号，代表数组；可以是空串，代表简单变量
/// 其文法为 idTail: T_L_PAREN realParamList? T_R_PAREN | (T_L_BRACKET expr T_R_BRACKET)* | ε
/// @return ast_node*
///
static ast_node * idTail(var_id_attr & id)
{
	...
	if (match(T_L_PAREN)) {
		...
		// 数组访问，idTail: (T_L_BRACKET expr T_R_BRACKET)+

		// 创建数组下标节点列表
		ast_node * indexNodesRoot = ast_node::New(ast_operator_type::AST_OP_ARRAY_DIMS);

		// 循环解析每个维度的下标表达式
		while (match(T_L_BRACKET)) {
			// 识别下标表达式
			ast_node * indexExpr = expr();
			if (!indexExpr) {
				semerror("数组下标必须是表达式");
				break;
			}
			...
			// 添加下标表达式到根节点
			if (!indexNodesRoot->insert_son_node(indexExpr)) {
				semerror("添加数组下标失败");
				break;
			}
		...
		// 创建数组访问节点
		node = ast_node::createArrayAccessNode(node, indexNodesRoot);
	} else {
		// 变量ID，idTail -> ε
...
/// 改造后的文法为：
/// unaryExp: T_DIGIT | T_L_PAREN expr T_R_PAREN | T_ID idTail | T_SUB unaryExp
...
static ast_node * unaryExp()
{
	...
	if (match(T_SUB)) {
		...
		// 单目求负，按“无符号数 + 求负符号”解释
		ast_node * rightNode = unaryExp();
		if (!rightNode) {
			return nullptr;
		}

		digit_int_attr zeroAttr{0, rd_line_no};
		ast_node * zeroNode = ast_node::New(zeroAttr);
		node = ast_node::New(ast_operator_type::AST_OP_SUB, zeroNode, rightNode);
	} else if (F(T_DIGIT)) {
...
///
/// @brief 解析数组维度，文法: arrayDims: T_L_BRACKET T_DIGIT T_R_BRACKET (T_L_BRACKET T_DIGIT T_R_BRACKET)*
/// @return 包含所有维度信息的AST节点
///
static ast_node * parseArrayDims()
{
	...
	// 创建维度节点根
	dimsNode = ast_node::New(ast_operator_type::AST_OP_ARRAY_DIMS);
	...
	while (match(T_L_BRACKET)) {
		...
		if (F(T_DIGIT)) {
			// 创建维度数值节点
			ast_node * dimNode = ast_node::New(rd_lval.integer_num);
			advance();
			
			// 添加到维度节点
			if (!dimsNode->insert_son_node(dimNode)) {
				semerror("添加数组维度失败");
				break;
			}
		} else {
...
static void varDeclList(ast_node * vardeclstmt_node)
{
	if (match(T_COMMA)) {
		...
		if (F(T_ID)) {
			...
			// 新建变量声明节点并加入变量声明语句中
			if (dimsNode) {
				(void) ast_node::add_var_decl_node(vardeclstmt_node, id, dimsNode);
			} else {
				(void) ast_node::add_var_decl_node(vardeclstmt_node, id);
			}
